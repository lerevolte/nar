...
<div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px;margin-top:16px;">
    ...
    <p style="font-size:13px;color:var(--text-secondary);margin-bottom:16px;">Понравилась песня? Сделайте её длиннее, перепойте в другом стиле, замените строчку или смешайте с другим треком — в Студии.</p>
    <div class="track-ops-grid">
        <a class="chart-add-btn" href="{{ route('studio') }}?op=upload_cover&song={{ $song->id }}">Кавер в новом стиле</a>
        <a class="chart-add-btn" href="{{ route('studio') }}?op=upload_extend&song={{ $song->id }}">Продлить трек</a>
        <a class="chart-add-btn" href="{{ route('studio') }}?op=replace_section&song={{ $song->id }}">Заменить фрагмент</a>
        <a class="chart-add-btn" href="{{ route('studio') }}?op=add_vocals&song={{ $song->id }}">Новый вокал на минус</a>
        <a class="chart-add-btn" href="{{ route('studio') }}?op=add_instrumental&song={{ $song->id }}">Музыка под голос</a>
        <a class="chart-add-btn" href="{{ route('studio') }}?op=mashup&song={{ $song->id }}">Мэшап с другим треком</a>
    </div>
    <p style="font-size:12px;color:var(--text-tertiary);margin-top:10px;">Каждая операция — 1 песня с баланса.</p>
</div>
@endif
